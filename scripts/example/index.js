    ...
    try {
        ...
        for (let player of players) {                                                                     //scorecheck for vanilla api
            ...
            for (let i = 0; i < playerInventory.size; i++) {
                ...
                if(player.hasTag('staffapi')) { return } else {
            ...
            if(player.hasTag('staffapi')) { return } else {
                if (acmbool && itemArray.length) {
                    overworld.runCommand(`tellraw @a {"rawtext":[{"text":"§¶§6§lConsole ► §6Anti-CBE §d${name} §bwas temp-kicked for having §c${itemArray}"}]}`);
                    player.runCommand('function console/asset/cbeitem_gt_warn');
                }
            }
        }
    } catch (error) {
