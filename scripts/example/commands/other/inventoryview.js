    ...
    try {
        ...
        if (sender.hasTag('staffapi')) {
            if (!args[0]) {
                ...
                //sender.tellraw(`§¶§6Console ► §c§lplayerfound : ${playerfound.getName()}`);

                // dump armor
                tellrawStaff(`§d${playername}'s§b inventory checked by §d${sender.name}`);
                sender.runCommand(`execute "${playername}" ~~~ function console/asset/enchanted_armor_check`);
                ...
                sender.tellraw(`§¶§6Console ► §c§lError 7: command failure`);
            }
        } else {
            sender.tellraw(`§¶§6Console ► §c§lError 4: Only Staff can use this command`);
        }
        
    } catch (error) {
        sender.tellraw(`§¶§6Console ► §c§lError 8: command failure`);
