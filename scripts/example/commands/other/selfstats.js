...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    try {
        ...
        const ComStringStats = `function console/itemcommand/playerstats`;
        ...
        const PlayerStringStats = {
            one: `execute "${args[1]}" ~~~ function console/itemcommand/playerstats`,
            two: `execute "${args[1]} ${args[2]}" ~~~ function console/itemcommand/playerstats`,
            three: `execute "${args[1]} ${args[2]} ${args[3]}" ~~~ function console/itemcommand/playerstats`,
            four: `execute "${args[1]} ${args[2]} ${args[3]} ${args[4]}" ~~~ function console/itemcommand/playerstats`
        };
        if (sender.scoreTest('icmtoggle') === 0) {
            ...
            if (cmdUsage.includes(args[0])) {
                if (!args[1]) {
                    ...
                    sender.tellraw(`§¶§6Console ► §c§lPlease Provide Player Name`);
                    ...
                    if (PlayerArgString.one) {
                        ...
                        sender.removeTag('stats_temp');
                        return tellrawStaff(`§¶§6Console ► §d${name} §bused stats §d${args.join(' ')}`);
                    }
                    else {
                        sender.tellraw(`§¶§6Console ► §c§lNo player by that name`);
                    }
                }
                    ...
                    if (PlayerArgString.two) {
                        ...
                        return sender.tellraw(`§¶§6Console ► §c§lNo player by that name`);
                    }
                }
                    ...
                    sender.tellraw(`§¶§6Console ► §c§lError : Did not Understand Player Name`);
                ...
                sender.removeTag('stats_temp');
                sender.runCommand(`playsound note.pling @s ~ ~ ~`);
                return tellrawStaff(`§¶§6Console ► §d${name} §bchecked their stats`);
            } else {
                return sender.tellraw(`§¶§6Console ► §c§lError : Command Failed`);
            }
        }
    } catch (error) {
