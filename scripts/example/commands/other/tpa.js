...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    if (sender.scoreTest('icmtoggle') === 0) {
        ...
        return sender.tellraw(`§¶§6Console ► §6TPA §cunavailable §bwhile in combat`);
    } else if (sender.scoreTest('tp_cooldown') != 0) {
        return sender.tellraw(`§¶§6Console ► §6TPA §cunavailable §bwhile warp commands are in cooldown. Please wait 40 seconds.`);
         ...
         if (registerInformation.name.match(chatmsg)) {
            if (tpsopen.includes(args[0])) {
                if (sender.hasTag('has_tpa')) {
                    return sender.tellraw(`§¶§6Console ► §bTPA Channel already created! Your Channel §7:§c "${sender.scoreTest('tpa')}" `);
                } else {
                    sender.runCommand(`scoreboard players random @s tpa 0 11`);
                    sender.runCommand(`tag @s add has_tpa`);
                    sender.tellraw(`§¶§6Console ► §bTPA Channel "${sender.scoreTest('tpa')}" was created`);
                    tellrawStaff(`§¶§6Console ► §d${name} §bopened a tpa channel`);
                }
            }
            else if (tpsclose.includes(args[0])) {
                if (!sender.hasTag('has_tpa')) {
                    return sender.tellraw(`§¶§6Console ► §bThere was no TPA channel to close" `);
                } else {
                    sender.tellraw(`§¶§6Console ► §bTPA Channel "${sender.scoreTest('tpa')}" was closed`);
                    sender.runCommand(`scoreboard players reset @s tpa`);
                    sender.runCommand(`tag @s remove has_tpa`);
                    tellrawStaff(`§¶§6Console ► §d${name} §bclosed their tpa channel manually`);
                }
            }
            else if (tpaIntString.includes(args[0])) { 
                sender.runCommand(`playsound note.pling @s ~ ~ ~`);
                sender.runCommand(`tellraw "${sender.name}" {"rawtext":[{"text":"§¶§6Console ► §6TPA §7: §bSuccessfully teleported to §6"},{"selector":"@p[scores={tpa=${args[0]}}]"}]}`);
                ...
                sender.runCommand(`tellraw @a[tag=staffapi] {"rawtext":[{"text":"§¶§6Console ► §d${name} §bteleported to §d"},{"selector":"@p[scores={tpa=${args[0]}}]"},{"text":" §bvia §eTPA"}]}`);
                sender.runCommand(`execute @p[scores={tpa=${args[0]}}] ~~~ tellraw @s {"rawtext":[{"text":"§¶§6Console ► §6TPA §7: §5${name} §bhas §bSuccessfully teleported! Your TPA Channel is now closed."}]}`);
                sender.runCommand(`execute @p[scores={tpa=${args[0]}}] ~~~ scoreboard players reset @s tpa`);
            } else {
                return sender.tellraw(`§¶§6Console ► §cERROR! §6Usage Example §7:§b§l !tpa [ open | close | number ]`);
            }
        }
        else {
            return sender.tellraw(`§¶§6Console ► §cERROR 2! §6Usage Example §7:§b§l !tpa [ open | close | number ]`);
        }
    }
});